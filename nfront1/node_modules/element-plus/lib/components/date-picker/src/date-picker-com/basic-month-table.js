  ...
  setup(__props, { expose, emit }) {
    ...
    const handleMonthTableClick = (event) => {
      ...
      if (props.selectionMode === "months") {
        ...
        const newValue = style.hasClass(target, "current") ? arrays.castArray(props.parsedValue).filter((d) => Number(d) !== Number(newMonth)) : arrays.castArray(props.parsedValue).concat([dayjs__default["default"](newMonth)]);
        emit("pick", newValue);
